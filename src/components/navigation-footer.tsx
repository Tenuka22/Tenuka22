...
const ZedIcon = ({ className }: { className?: string }) => (
...
const Row = ({
  ...
  <motion.button
    ...
    className="text-foreground relative flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-medium"
  >
    <motion.span
      variants={{
...
const Detail = ({ row, onBack }: { row: MenuRow; onBack: () => void }) => (
  <div className="flex w-[17.125rem] flex-col gap-3 p-1">
    <motion.button
      ...
      whileTap={{ scale: 0.94 }}
      transition={SPRING_PRESS}
      className="text-muted-foreground hover:bg-muted hover:text-foreground flex w-fit items-center gap-1.5 rounded-lg px-2 py-1 text-xs font-medium"
    >
    ...
    <div className="flex items-center gap-2 px-2">
      <row.icon className="text-muted-foreground h-4 w-4" />
      <span className="text-foreground text-sm font-medium">{row.label}</span>
    </div>

    {row.code ? (
      <pre className="bg-muted text-foreground overflow-x-auto rounded-lg px-3 py-2 font-mono text-xs">
        {row.code}
      </pre>
    ) : null}
